...
namespace ArmoSystems.ArmoGet.VariablesManager
{
    internal static class VariablesManager
    {
        ...
        public static void UpdateVariables()
        {
            ...
            var variablesForSet =
                variablesFiles.SelectMany( GetFileVariables ).Concat( GetCalculateVariables() ).GroupBy( variable => variable.Name ).Select( group => group.OrderBy( variable => variable.Priority ).Last() ).ToList();
        ...
        {
            return ( variableFileName.FileName == VariablesFile.DefaultTextFile ? 0 : variableFileName.FileName.Length ) + ( variableBranchName == FilesHelper.DefaultBranchName ? 1 : 2 );
        }
